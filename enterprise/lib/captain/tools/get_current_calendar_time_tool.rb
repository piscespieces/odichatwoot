class Captain::Tools::GetCurrentCalendarTimeTool < Captain::Tools::BasePublicTool
  ...
  param :timezone,
        type: 'string',
        desc: "Optional IANA timezone (e.g., 'America/Los_Angeles', 'Europe/London', 'UTC'). If not provided, uses the primary Google Calendar's default timezone.",
        required: false
  ...
  def perform(tool_context, timezone: nil)
    ...
    target_timezone = get_calendar_timezone(conversation) if target_timezone.blank? && conversation
